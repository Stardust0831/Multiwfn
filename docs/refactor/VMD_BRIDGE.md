...
`multiwfn_resolve_path` was defined. If `tclsh` is not installed, the script
prints a skip message and exits successfully. The GNU noGUI smoke target uses it
on the VMD scenes generated by the real `Multiwfn_noGUI` structure and cube
export fixtures.
