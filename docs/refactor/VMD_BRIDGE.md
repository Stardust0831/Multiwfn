...
not installed, the script prints a skip message and exits successfully. The
narrow VMD bridge smoke test uses it on all generated scene variants, and the
GNU noGUI smoke target uses it on the VMD scenes generated by the real
`Multiwfn_noGUI` structure, loaded-cube, and wavefunction-grid export fixtures.
