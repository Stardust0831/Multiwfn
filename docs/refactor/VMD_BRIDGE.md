...
tools/vmd-scene-source-check.sh --require-tcl path/to/scene.vmd.tcl
```
...
successfully unless `--require-tcl` is used. When a scene fails, the diagnostic
includes the scene path before the Tcl error so batch checks identify the
failing file. The narrow VMD bridge smoke test uses it on all generated scene
variants and includes a negative fixture for a scene that defines the resolver
but loads no data. The GNU noGUI smoke target uses it on the VMD scenes
generated by the real `Multiwfn_noGUI` structure, loaded-cube, VASP grid, and
wavefunction-grid export fixtures.
