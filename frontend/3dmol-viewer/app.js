    ...
    if (select === els.guiOrbitalSelect) {
      ...
      if (state.orbitals.items.length) {
        state.orbitals.items.forEach((orbital) => {
          ...
          option.value = layer ? String(layer.id) : `orbital:${orbital.index}`;
...
function setActiveGuiLayer(layerId, options = {}) {
  ...
  if (id.startsWith('orbital:')) {
    ...
    els.guiOrbitalInput.value = Number.isFinite(orbitalIndex) ? String(orbitalIndex) : '0';
    setStatus('Orbital cube has not been generated by Multiwfn yet', false);
    return;
  }
...
function bindEvents() {
  ...
  els.guiOrbitalInput.addEventListener('change', () => {
    ...
    const layer = layerForOrbital(index) || (!state.orbitals.count ? state.layers[index - 1] : null);
    if (layer) setActiveGuiLayer(layer.id, { orbitalSelection: true });
    else if (state.orbitals.count && index <= state.orbitals.count) setStatus('Orbital cube has not been generated by Multiwfn yet', false);
    else setStatus('Orbital index out of range', false);
  });
