...
pub fn embedded_key_registry() -> Result<Vec<PublicKeyEntry>> {
    let source =
        option_env!("MULTIWFN_UPDATER_TRUSTED_KEYS_JSON").unwrap_or(EMBEDDED_PUBLIC_KEYS_JSON);
    let keys: Vec<PublicKeyEntry> = serde_json::from_str(source)?;
    if keys.is_empty() {
        return Err(Error::Signature("trusted key registry is empty".into()));
    }
    Ok(keys)
}
...
pub fn parse_tag(tag: &str) -> Option<u64> {
    parse_preview_tag(tag)
}

pub fn is_newer_preview(candidate: &str, current: Option<&str>) -> bool {
    let Some(c) = parse_preview_tag(candidate) else {
        return false;
    };
    match current.and_then(parse_preview_tag) {
        Some(v) => c > v,
        None => true,
    }
}
...
pub fn sign_manifest(
    ...
    if key_id.trim().is_empty() {
        return Err(Error::Invalid("empty key id".into()));
    }
    let text = Zeroizing::new(pkcs8_base64.trim().to_owned());
    let der = Zeroizing::new(
        B64.decode(text.as_bytes())
            .map_err(|e| Error::Invalid(format!("private key base64: {e}")))?,
    );
    let signing = SigningKey::from_pkcs8_der(&der)
        .map_err(|e| Error::Invalid(format!("PKCS#8 private key: {e}")))?;
...
pub fn build_signed_manifest(
    ...
    sign_manifest(manifest, key_id, pkcs8_base64)
}
...
pub fn verify_signed_manifest(
    ...
    let entry = registry
        .iter()
        .find(|e| e.version == 1 && e.key_id == signed.key_id)
        .ok_or_else(|| Error::Signature(format!("unknown key id {}", signed.key_id)))?;
    let key_bytes = B64
        .decode(entry.public_key.as_bytes())
        .map_err(|e| Error::Signature(format!("public key encoding: {e}")))?;
    ...
    let sig_bytes = B64
        ...
        .map_err(|e| Error::Signature(format!("signature encoding: {e}")))?;
    let sig = Signature::from_slice(&sig_bytes).map_err(|e| Error::Signature(e.to_string()))?;
    key.verify(&canonical_json(&signed.manifest)?, &sig)
...
pub fn verify_release_manifest(
    ...
    verify_signed_manifest(signed, registry)
}

fn proof_payload(proof: &InstalledInventoryProof) -> InstalledInventoryPayload<'_> {
    InstalledInventoryPayload {
        version: proof.version,
        repository: &proof.repository,
        channel: &proof.channel,
        release_tag: &proof.release_tag,
        target: &proof.target,
        install_manifest_sha256: &proof.install_manifest_sha256,
    }
}

pub fn sign_inventory_proof(
    ...
    let signing = SigningKey::from_pkcs8_der(&der)
        .map_err(|e| Error::Invalid(format!("PKCS#8 private key: {e}")))?;
    let mut proof = InstalledInventoryProof {
        version: 1,
        repository: REPOSITORY.into(),
        channel: CHANNEL.into(),
        release_tag: manifest.release_tag.clone(),
        target: manifest.target.clone(),
        install_manifest_sha256: manifest_sha256(manifest)?,
        key_id: key_id.into(),
        signature: String::new(),
    };
    proof.signature = B64.encode(
        signing
            .sign(&canonical_json(&proof_payload(&proof))?)
            .to_bytes(),
    );
    Ok(proof)
}
...
pub fn verify_inventory_proof(
    ...
    let entry = registry
        .iter()
        .find(|e| e.version == 1 && e.key_id == proof.key_id)
        .ok_or_else(|| Error::Signature(format!("unknown proof key {}", proof.key_id)))?;
    let bytes = B64
        .decode(entry.public_key.as_bytes())
        .map_err(|e| Error::Signature(e.to_string()))?;
    let key_array: [u8; 32] = bytes
        .try_into()
        .map_err(|_| Error::Signature("proof public key length".into()))?;
    let key = VerifyingKey::from_bytes(&key_array).map_err(|e| Error::Signature(e.to_string()))?;
    let sig = Signature::from_slice(
        &B64.decode(proof.signature.as_bytes())
            .map_err(|e| Error::Signature(e.to_string()))?,
    )
    .map_err(|e| Error::Signature(e.to_string()))?;
    key.verify(&canonical_json(&proof_payload(proof))?, &sig)
        .map_err(|e| Error::Signature(e.to_string()))
}
...
fn validate_inventory_proof_metadata(proof: &InstalledInventoryProof) -> Result<()> {
    if proof.version != 1
        ...
        || proof.target.trim().is_empty()
        || parse_preview_tag(&proof.release_tag).is_none()
    {
        return Err(Error::Signature("invalid installed proof metadata".into()));
    }
    validate_digest(&proof.install_manifest_sha256)
}
...
pub fn read_inventory_proof(root: &Path) -> Result<InstalledInventoryProof> {
    ...
    if bytes.len() > MAX_JSON_BYTES {
        return Err(Error::Limit("inventory proof exceeds 64 KiB".into()));
    }
...
fn validate_release_manifest(m: &ReleaseManifestV1) -> Result<()> {
    ...
    if m.repository != REPOSITORY {
        return Err(Error::Invalid("manifest repository mismatch".into()));
    }
    if m.channel != CHANNEL || parse_preview_tag(&m.tag).is_none() {
        return Err(Error::Invalid("manifest is not a MatterViz preview".into()));
    }
    if m.targets.is_empty() || m.targets.len() > 64 {
        return Err(Error::Invalid("invalid target count".into()));
    }
    let mut targets = BTreeSet::new();
    for t in &m.targets {
        if t.target.trim().is_empty() || !targets.insert(t.target.clone()) {
        ...
        {
            return Err(Error::Invalid(
                "archive name must be a printable ASCII basename".into(),
            ));
        }
        if t.archive_size > MAX_ARCHIVE_BYTES {
            return Err(Error::Limit("archive exceeds 512 MiB".into()));
        }
        for digest in [&t.archive_sha256, &t.install_manifest_sha256] {
            ...
            {
                return Err(Error::Invalid(
                    "digest must be lowercase hexadecimal".into(),
                ));
            }
        }
    }
...
pub fn manifest_bytes(manifest: &InstallManifestV1) -> Result<Vec<u8>> {
    canonical_json(manifest)
}
pub fn manifest_sha256(manifest: &InstallManifestV1) -> Result<String> {
    Ok(sha256_hex(&manifest_bytes(manifest)?))
}
...
fn validate_digest(digest: &str) -> Result<()> {
    if digest.len() != 64
        || !digest
            ...
            .all(|b| b.is_ascii_digit() || (b'a'..=b'f').contains(&b))
    {
        return Err(Error::Invalid("invalid SHA-256 digest".into()));
    }
    Ok(())
}
...
pub fn validate_install_manifest(m: &InstallManifestV1) -> Result<()> {
    if m.version != 1 {
        return Err(Error::Invalid(
            "unsupported install manifest version".into(),
        ));
    }
    if parse_preview_tag(&m.release_tag).is_none() || m.target.trim().is_empty() {
        return Err(Error::Invalid(
            "invalid install manifest release/target".into(),
        ));
    }
    if m.files.len() > MAX_ENTRIES {
        return Err(Error::Limit("too many files".into()));
    }
    ...
    for f in &m.files {
        ...
        if f.path == INSTALL_MANIFEST_NAME || f.path == INSTALL_PROOF_NAME {
            return Err(Error::Invalid(
                "updater metadata cannot be listed in inventory".into(),
            ));
        }
        if f.path == "settings.ini" && f.policy != FilePolicy::Preserve {
            return Err(Error::Invalid("settings.ini must be preserve".into()));
        }
        if f.policy == FilePolicy::Preserve && f.path != "settings.ini" {
            return Err(Error::Invalid("only settings.ini may be preserve".into()));
        }
        if !seen.insert(f.path.clone()) {
            return Err(Error::Invalid(format!("duplicate path {}", f.path)));
        }
        let key = f.path.to_ascii_lowercase();
        if !folded.insert(key) {
            return Err(Error::Invalid(format!("casefold collision {}", f.path)));
        }
    }
    Ok(())
}
...
pub fn read_install_manifest(root: &Path) -> Result<InstallManifestV1> {
    ...
    if bytes.len() > MAX_JSON_BYTES {
        return Err(Error::Limit("install manifest exceeds 64 KiB".into()));
    }
...
fn check_regular_file(path: &Path) -> Result<fs::Metadata> {
    ...
    #[cfg(windows)]
    {
        ...
        if metadata.file_attributes() & 0x400 != 0 || windows_number_of_links(path)? > 1 {
            return Err(Error::Invalid(format!(
                "reparse point or hard link rejected: {}",
                path.display()
            )));
        }
    }
    ...
    match fs::symlink_metadata(path) {
        Ok(meta) => {
            if meta.file_type().is_symlink() {
                return Err(Error::Invalid(format!(
                    "symlink/reparse point rejected: {}",
                    path.display()
                )));
            }
            #[cfg(windows)]
...
fn collect_files(root: &Path, base: &Path, out: &mut Vec<InstallFile>) -> Result<()> {
    for item in fs::read_dir(root)? {
        ...
        if meta.file_type().is_symlink() {
            return Err(Error::Invalid(format!("symlink rejected: {rel}")));
        }
        #[cfg(windows)]
        {
            use std::os::windows::fs::MetadataExt;
        ...
        if meta.is_dir() {
            ...
            if rel == INSTALL_MANIFEST_NAME || rel == INSTALL_PROOF_NAME {
                continue;
            }
            ...
            let policy = if rel == "settings.ini" {
                FilePolicy::Preserve
            } else {
                FilePolicy::Managed
            };
            ...
            return Err(Error::Invalid(format!("special file rejected: {rel}")));
        }
...
pub fn inventory_directory(
    ...
    if !root.is_dir() {
        return Err(Error::Invalid("inventory root is not a directory".into()));
    }
...
pub fn generate_inventory(
    ...
    inventory_directory(root, target, release_tag)
}
...
pub fn verify_inventory_for_signing(root: &Path, manifest: &InstallManifestV1) -> Result<()> {
    ...
    for file in &actual.files {
        let Some(expected_file) = expected.get(file.path.as_str()) else {
            return Err(Error::Invalid(format!(
    ...
    for f in &manifest.files {
        let path = path_for(root, &f.path);
        if f.policy == FilePolicy::Preserve && existing_nolink(&path)?.is_none() {
            continue;
        }
        let (size, digest) = hash_file(&path)
            .map_err(|_| Error::Invalid(format!("missing or invalid file {}", f.path)))?;
...
pub fn authenticate_current_inventory(
    root: &Path,
    proof: &InstalledInventoryProof,
    registry: &[PublicKeyEntry],
    target: &str,
    ...
    for file in local
        .files
        .iter()
        .filter(|f| f.policy == FilePolicy::Managed)
    {
...
pub fn authenticate_target_inventory(
    target_root: &Path,
    ...
    if proof.repository != REPOSITORY
        || proof.channel != CHANNEL
        || proof.release_tag != target.release_tag
        || proof.target != target.target
...
pub fn extract_archive(bytes: &[u8], destination: &Path) -> Result<InstallManifestV1> {
    ...
    if proof.release_tag != file_manifest.release_tag
        || proof.target != file_manifest.target
        || manifest_sha256(&file_manifest)? != proof.install_manifest_sha256
    {
        return Err(Error::Invalid(
            "installed proof does not bind inventory".into(),
        ));
    }
    let expected = manifest_sha256(&file_manifest)?;
    let actual = sha256_hex(&canonical_json(&file_manifest)?);
    if expected != actual {
        return Err(Error::Invalid("manifest hash calculation failed".into()));
    }
    Ok(file_manifest)
}

fn normalize_archive_root(destination: &Path) -> Result<()> {
    if destination.join(INSTALL_MANIFEST_NAME).is_file() {
        return Ok(());
    }
    let mut children = fs::read_dir(destination)?;
    let Some(first) = children.next() else {
        return Err(Error::Invalid("archive is empty".into()));
    };
    let first = first?.path();
    if !first.is_dir() || children.next().is_some() {
        return Err(Error::Invalid(
            "archive has no unambiguous package root".into(),
        ));
    }
    for item in fs::read_dir(&first)? {
        ...
        if existing_nolink(&target)?.is_some() {
            return Err(Error::Invalid("package-root collision".into()));
        }
...
pub fn extract_archive_secure(bytes: &[u8], destination: &Path) -> Result<InstallManifestV1> {
...
fn extract_zip(bytes: &[u8], destination: &Path) -> Result<()> {
    ...
    for i in 0..archive.len() {
        if i >= MAX_ENTRIES {
            return Err(Error::Limit("too many archive entries".into()));
        }
        let mut entry = archive.by_index(i)?;
        if entry.name().contains('\\') {
            return Err(Error::Invalid("backslash archive path rejected".into()));
        }
        let raw = entry.name().to_owned();
        if raw.ends_with('/') {
            validate_rel_path(raw.trim_end_matches('/'))?;
            continue;
        }
        validate_rel_path(&raw)?;
        if !paths.insert(raw.clone()) || !folded.insert(raw.to_ascii_lowercase()) {
            return Err(Error::Invalid("duplicate archive path".into()));
        }
        if entry.unix_mode().is_some_and(|m| m & 0o170000 == 0o120000) {
            return Err(Error::Invalid("symlink archive entry".into()));
        }
        let size = entry.size();
        total = total
            .checked_add(size)
            .ok_or_else(|| Error::Limit("extracted size overflow".into()))?;
        if total > MAX_EXTRACTED_BYTES {
            return Err(Error::Limit("extracted data exceeds 2 GiB".into()));
        }
        let path = path_for(destination, &raw);
        ensure_parent(&path)?;
...
fn extract_tar(bytes: &[u8], destination: &Path) -> Result<()> {
        ...
        .map_err(|e| Error::Invalid(e.to_string()))?
    {
        ...
        let raw = raw_path
            .to_str()
            .ok_or_else(|| Error::Invalid("non-UTF-8 archive path rejected".into()))?
            .to_owned();
        if raw.contains('\\') {
            return Err(Error::Invalid("backslash archive path rejected".into()));
        }
        ...
        if !entry.header().entry_type().is_file() {
            return Err(Error::Invalid("tar link/special entry rejected".into()));
        }
        validate_rel_path(&raw)?;
        if !paths.insert(raw.clone()) || !folded.insert(raw.to_ascii_lowercase()) {
            return Err(Error::Invalid("duplicate archive path".into()));
        ...
        let copied = io::copy(&mut entry.by_ref().take(size), &mut out)?;
        if copied != size {
            return Err(Error::Invalid("truncated tar entry".into()));
        }
        #[cfg(unix)]
...
pub fn transaction_dir(install_root: &Path) -> PathBuf {
    ...
    install_root
        ...
        .join(format!(".{name}.multiwfn-updater-txn"))
}
...
pub fn candidate_dir(install_root: &Path) -> PathBuf {
    ...
    install_root
        .parent()
        .unwrap_or_else(|| Path::new("."))
        .join(format!(".{name}.multiwfn-updater-candidate"))
}

pub fn stage_dir(install_root: &Path) -> PathBuf {
    candidate_dir(install_root).join("stage")
}

fn validate_transaction_state(state: &TransactionState) -> Result<()> {
    let root = Path::new(&state.install_root);
    if state.version != 1
        || state.install_root.is_empty()
        || state.entries.len() > MAX_ENTRIES.saturating_mul(2)
    {
    ...
    if (state.lifecycle == "installed") != state.pending_confirm {
        return Err(Error::Conflict(
            "transaction lifecycle confirmation mismatch".into(),
        ));
    }
    if state.lifecycle == "installed"
        && (state.entries.is_empty() || state.entries.iter().any(|entry| entry.phase != "applied"))
    {
        return Err(Error::Conflict(
            "installed transaction is incomplete".into(),
        ));
    }
    ...
    for (index, entry) in state.entries.iter().enumerate() {
        ...
        if !matches!(
            ...
            "replace" | "remove" | "manifest" | "proof"
        ) {
            return Err(Error::Conflict("invalid transaction operation".into()));
        }
        if !matches!(
            entry.phase.as_str(),
            "" | "planned" | "backedUp" | "applied"
        ) {
            return Err(Error::Conflict("invalid transaction phase".into()));
        }
        if let Some(backup) = &entry.backup {
            if backup != &backup_path(&txn, index).display().to_string() {
                return Err(Error::Conflict("transaction backup escapes journal".into()));
            }
        }
...
pub fn load_transaction(install_root: &Path) -> Result<Option<TransactionState>> {
    ...
    let Some(meta) = existing_nolink(&dir)? else {
        return Ok(None);
    };
    if !meta.is_dir() {
        return Err(Error::Conflict(
            "transaction path is not a directory".into(),
        ));
    }
    let path = dir.join("journal.json");
    if !path.is_file() {
        return Err(Error::Conflict("transaction journal is missing".into()));
    }
    ...
    if state.install_root != install_root.display().to_string() {
        return Err(Error::Conflict("invalid transaction journal root".into()));
    }
...
fn backup_path(txn: &Path, index: usize) -> PathBuf {
    txn.join("backups").join(format!("{index:016x}"))
}
...
fn move_replace(
    ...
    let backup = if existing_nolink(&destination)?.is_some() {
        Some(backup_path(txn, state.entries.len()).display().to_string())
    } else {
        None
    };
    let index = journal_entry(
        state,
        txn,
        operation,
        rel,
...
fn move_remove(state: &mut TransactionState, txn: &Path, root: &Path, rel: &str) -> Result<()> {
    let destination = path_for(root, rel);
    if existing_nolink(&destination)?.is_none() {
        return Ok(());
    }
    ...
    while let Some(directory) = parent.filter(|directory| *directory != root) {
        match fs::remove_dir(directory) {
            Ok(()) => {
                if let Some(grandparent) = directory.parent() {
...
pub fn apply_transaction(
    ...
    for f in &target.files {
        ...
        if f.policy == FilePolicy::Managed
            && existing_nolink(&path)?.is_some()
            && !current_map.contains_key(&f.path)
        {
            return Err(Error::Conflict(format!(
        ...
        if f.policy == FilePolicy::Preserve {
            if let Some(meta) = existing_nolink(&path)? {
                if !meta.is_file() {
                    return Err(Error::Conflict(format!(
                        "preserve path is not a regular file {}",
                        f.path
                    )));
                }
            }
    ...
    let txn = transaction_dir(install_root);
    if existing_nolink(&txn)?.is_some() {
    ...
    let mut state = TransactionState {
        ...
        target_tag: target.release_tag.clone(),
        entries: Vec::new(),
        lifecycle: "applying".into(),
    ...
    let result = (|| {
        for f in &target.files {
            let path = path_for(install_root, &f.path);
            if f.policy == FilePolicy::Preserve && existing_nolink(&path)?.is_some() {
                continue;
            }
...
pub fn apply_authenticated_transaction(
    install_root: &Path,
    target_root: &Path,
    target: &InstallManifestV1,
...
pub fn rollback_state(state: &TransactionState) -> Result<()> {
    ...
    for entry in state.entries.iter().rev() {
        ...
        let should_remove = if entry.backup.is_some() {
            ...
            destination_exists
                && (entry.phase == "applied" || (entry.phase == "planned" && staged_source_missing))
        };
        if should_remove && destination_exists {
            let meta = existing_nolink(&path)?.ok_or_else(|| {
                Error::Conflict(format!("rollback path disappeared: {}", path.display()))
            })?;
            if !meta.is_file() {
                return Err(Error::Conflict(format!(
                    "rollback encountered non-file: {}",
                    path.display()
                )));
            }
            fs::remove_file(&path)?;
...
pub fn confirm_transaction(install_root: &Path) -> Result<()> {
    let Some(state) = load_transaction(install_root)? else {
        return Err(Error::Invalid("no pending transaction".into()));
    };
    if state.lifecycle != "installed" || !state.pending_confirm {
        return Err(Error::Conflict(
            "transaction is not durably installed".into(),
        ));
    }
...
pub fn rollback_last(install_root: &Path) -> Result<()> {
    let Some(state) = load_transaction(install_root)? else {
        return Err(Error::Invalid("no transaction to roll back".into()));
    };
...
pub fn state_path(install_root: &Path) -> PathBuf {
    candidate_dir(install_root).join("candidate.json")
}
pub fn save_candidate(install_root: &Path, candidate: &CandidateState) -> Result<()> {
    if candidate.install_root != install_root.display().to_string() {
        return Err(Error::Invalid("candidate install root mismatch".into()));
    }
...
pub fn load_candidate(install_root: &Path) -> Result<Option<CandidateState>> {
    ...
    if candidate.install_root != install_root.display().to_string() {
        return Err(Error::Conflict("candidate install root mismatch".into()));
    }
    Ok(Some(candidate))
}
    ...
    match existing_nolink(&dir)? {
        Some(meta) if meta.is_dir() => {
            ...
            for item in fs::read_dir(&dir)? {
                ...
                let name = name
                    .to_str()
                    .ok_or_else(|| Error::Conflict("candidate contains a non-UTF-8 path".into()))?;
...
impl GitHubClient {
    pub fn new() -> Result<Self> {
        let client = reqwest::blocking::Client::builder()
            .user_agent("Multiwfn-MatterViz-Updater/1")
            .timeout(Duration::from_secs(30))
            .redirect(reqwest::redirect::Policy::custom(|attempt| {
                if attempt.previous().len() >= 5 || !GitHubClient::allowed(attempt.url().as_str()) {
                    attempt.stop()
                } else {
                    attempt.follow()
                }
    ...
    pub fn download_bounded(&self, url: &str, limit: u64) -> Result<Vec<u8>> {
        if !Self::allowed(url) {
            return Err(Error::Network("download host is not allowlisted".into()));
        }
        let response = self
            .client
            .get(url)
        ...
        if response.content_length().is_some_and(|size| size > limit) {
            return Err(Error::Limit("download exceeds configured limit".into()));
        }
...
pub fn select_latest_release<'a>(
    ...
    releases
        .iter()
        .filter(|r| r.prerelease && is_newer_preview(&r.tag_name, current))
        .max_by_key(|r| parse_preview_tag(&r.tag_name).unwrap_or(0))
}

pub fn json_output<T: Serialize>(value: &T) -> Result<String> {
    let text = serde_json::to_string(value)?;
    if text.len() > MAX_JSON_BYTES {
        return Err(Error::Limit("JSON output exceeds 64 KiB".into()));
    }
    Ok(text)
}
...
mod tests {
    ...
    #[test]
    fn tags_are_strict_and_ordered() {
        ...
        assert!(is_newer_preview(
            "matterviz-preview-2",
            Some("matterviz-preview-1")
        ));
    }

    #[test]
    fn manifest_rejects_unsafe_paths() {
        let mut m = InstallManifestV1 {
            version: 1,
            target: "x".into(),
            release_tag: "matterviz-preview-1".into(),
            files: vec![],
        };
        m.files.push(InstallFile {
            path: "../x".into(),
            size: 0,
            sha256: "0".repeat(64),
            executable: false,
    ...
    #[test]
    fn inventory_preserves_settings_policy() {
        let d = tempdir().unwrap();
        fs::write(d.path().join("settings.ini"), b"x").unwrap();
        let m = inventory_directory(d.path(), "linux-x86_64", "matterviz-preview-1").unwrap();
        assert_eq!(m.files[0].policy, FilePolicy::Preserve);
    }

    #[test]
    fn inventories_reject_hard_links() {
        let d = tempdir().unwrap();
        let managed = d.path().join("managed");
        fs::write(&managed, b"payload").unwrap();
        fs::hard_link(&managed, d.path().join("alias")).unwrap();
        assert!(matches!(
            inventory_directory(d.path(), "linux-x86_64", "matterviz-preview-1"),
            Err(Error::Invalid(message)) if message.contains("hard link")
        ));
    }

    #[test]
    fn signed_manifest_tampering_and_rotation_fail_closed() {
        ...
        let manifest = ReleaseManifestV1 {
            version: 1,
            repository: REPOSITORY.into(),
            tag: "matterviz-preview-2".into(),
            channel: CHANNEL.into(),
            targets: vec![ReleaseTarget {
                target: "linux-x86_64".into(),
                archive_name: "a.tar.gz".into(),
                archive_size: 1,
                archive_sha256: "a".repeat(64),
                install_manifest_sha256: "b".repeat(64),
            }],
        };
        ...
        tampered.manifest.tag = "matterviz-preview-3".into();
        assert!(verify_signed_manifest(&tampered, &registry).is_err());
        assert!(verify_signed_manifest(&signed, &[]).is_err());
    }

    #[test]
    fn inventory_proof_binds_full_manifest_but_allows_installed_settings_change() {
        use ed25519_dalek::pkcs8::EncodePrivateKey;
        let signing = SigningKey::from_bytes(&[9u8; 32]);
        let key = B64.encode(signing.to_pkcs8_der().unwrap().as_bytes());
        let mut manifest = InstallManifestV1 {
            version: 1,
            target: "linux-x86_64".into(),
            release_tag: "matterviz-preview-1".into(),
            files: vec![
                ...
                InstallFile {
                    path: "settings.ini".into(),
                    size: 1,
                    sha256: sha256_hex(b"a"),
                    executable: false,
                    policy: FilePolicy::Preserve,
                },
            ],
        };
        ...
        assert_ne!(
            managed_manifest_sha256(&manifest).unwrap(),
            proof.install_manifest_sha256
        );
        let mut tampered = proof;
        tampered.target = "windows-x86_64".into();
    ...
    fn stage_with_manifest(root: &Path, manifest: &InstallManifestV1, files: &[(&str, &[u8])]) {
        ...
        fs::write(state_path(root), b"owned test candidate").unwrap();
        ...
        fs::write(
            stage.join(INSTALL_MANIFEST_NAME),
    ...
    fn archive_fixture(root: &Path) -> InstallManifestV1 {
        ...
        fs::write(
            root.join(INSTALL_MANIFEST_NAME),
            manifest_bytes(&manifest).unwrap(),
        )
        .unwrap();
        let proof = InstalledInventoryProof {
    ...
    fn extracts_flat_zip_and_single_root_tar_packages() {
        ...
        tar.append_dir_all("Multiwfn-MatterViz-preview", source.path())
            .unwrap();
    ...
    #[test]
    fn candidate_stage_coexists_with_active_transaction_path() {
        let d = tempdir().unwrap();
    ...
    #[test]
    fn candidate_cleanup_preserves_unknown_files() {
        let d = tempdir().unwrap();
        ...
        assert!(matches!(clear_candidate(d.path()), Err(Error::Conflict(_))));
        assert_eq!(
            fs::read(candidate.join("user-sentinel.txt")).unwrap(),
            b"keep"
        );
    }

    #[test]
    fn transaction_preserves_settings_and_removes_old_managed_file() {
        use ed25519_dalek::pkcs8::EncodePrivateKey;
        ...
        let current = InstallManifestV1 {
            ...
            release_tag: "matterviz-preview-1".into(),
            files: vec![
                file_entry("app", b"old", FilePolicy::Managed),
                file_entry("gone", b"remove", FilePolicy::Managed),
                file_entry("settings.ini", b"package", FilePolicy::Preserve),
            ],
        };
        fs::write(
            d.path().join(INSTALL_MANIFEST_NAME),
            manifest_bytes(&current).unwrap(),
        )
        .unwrap();
        ...
        let current_proof = sign_inventory_proof(&current, "k", &key).unwrap();
        fs::write(
            d.path().join(INSTALL_PROOF_NAME),
            serde_json::to_vec(&current_proof).unwrap(),
        )
        .unwrap();
        let target = InstallManifestV1 {
            ...
            release_tag: "matterviz-preview-2".into(),
            files: vec![
                file_entry("app", b"new", FilePolicy::Managed),
                file_entry("added", b"official", FilePolicy::Managed),
                file_entry("settings.ini", b"package-new", FilePolicy::Preserve),
            ],
        };
        stage_with_manifest(
            d.path(),
            ...
            &[
                ("app", b"new"),
                ("added", b"official"),
                ("settings.ini", b"package-new"),
            ],
        );
        let target_proof = sign_inventory_proof(&target, "k", &key).unwrap();
        fs::write(
            stage_dir(d.path()).join(INSTALL_PROOF_NAME),
            serde_json::to_vec(&target_proof).unwrap(),
        )
        .unwrap();
        let state = apply_authenticated_transaction(
            d.path(),
            stage_dir(d.path()).as_path(),
            &target,
            &current_proof,
            &registry,
        )
        ...
        assert_eq!(
            fs::read(d.path().join("user-sentinel.txt")).unwrap(),
            b"keep"
        );
    ...
    #[test]
    fn modified_managed_file_and_new_path_collision_abort_before_writes() {
        let d = tempdir().unwrap();
        ...
        let current = InstallManifestV1 {
            ...
            target: "linux-x86_64".into(),
            release_tag: "matterviz-preview-1".into(),
            files: vec![file_entry("app", b"old", FilePolicy::Managed)],
        };
        fs::write(
            d.path().join(INSTALL_MANIFEST_NAME),
            manifest_bytes(&current).unwrap(),
        ...
        assert!(matches!(
            apply_transaction(d.path(), stage_dir(d.path()).as_path(), &target),
        ...
        let current2 = InstallManifestV1 {
            version: 1,
            target: "linux-x86_64".into(),
            release_tag: "matterviz-preview-1".into(),
            files: vec![],
        ...
        fs::write(
            d2.path().join(INSTALL_MANIFEST_NAME),
            manifest_bytes(&current2).unwrap(),
        )
        .unwrap();
        let target2 = InstallManifestV1 {
            version: 1,
            target: "linux-x86_64".into(),
            release_tag: "matterviz-preview-2".into(),
            files: vec![file_entry("collision", b"package", FilePolicy::Managed)],
        };
        stage_with_manifest(d2.path(), &target2, &[("collision", b"package")]);
        assert!(matches!(
            apply_transaction(d2.path(), stage_dir(d2.path()).as_path(), &target2),
            Err(Error::Conflict(_))
        ));
    ...
    #[test]
    fn installed_settings_content_changes_are_allowed_but_manifest_tamper_is_not() {
        use ed25519_dalek::pkcs8::EncodePrivateKey;
        let d = tempdir().unwrap();
        ...
        let manifest = InstallManifestV1 {
            ...
            release_tag: "matterviz-preview-1".into(),
            files: vec![
                file_entry("app", b"x", FilePolicy::Managed),
                file_entry("settings.ini", b"package-settings", FilePolicy::Preserve),
            ],
        };
        fs::write(
            d.path().join(INSTALL_MANIFEST_NAME),
    ...
    #[test]
    fn rollback_failure_retains_recovery_journal() {
        let d = tempdir().unwrap();
    ...
    #[test]
    fn applying_journal_is_recovery_and_cannot_be_confirmed() {
        let d = tempdir().unwrap();
        let txn = transaction_dir(d.path());
