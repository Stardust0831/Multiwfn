...
import { parse_plot_artifact, to_matterviz_series } from '../src/plot.ts'
...
test('parses DOS multi-panel sticks and preserves explicit ranges', () => {
  ...
  assert.deepEqual(to_matterviz_series(parsed.panels[0].series[0]).x, [-1, -1, -1, 1, 1, 1])
  assert.deepEqual(to_matterviz_series(parsed.panels[0].series[0]).y, [0, 2, 0, 0, 4, 0])
})

test('maps captured labels to invisible ScatterPlot point anchors', () => {
  ...
  assert.equal(sticks.markers, 'line+points')
  assert.deepEqual(sticks.point_label, [{}, { text: '1H', auto_placement: true }, {}, {}, {}, {}])
})
